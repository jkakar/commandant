...
# --------------------------------------------------------------------
# Extensions for use with testresources

try:
    from testresources import TestResource

    class MockerResource(TestResource):

        def make(self, dependency_resources):
            return Mocker()

        def clean(self, mocker):
            if mocker.is_recording() and mocker.get_events():
                raise RuntimeError(
                    "Mocker must be put in replay mode with mocker.replay().")
            mocker.verify()
            mocker.restore()
except ImportError:
    pass
